...
*AWS 콘솔 상에서는 Event들이 Succeeded로 보이지만, 실제 스크립 실행(ApplicationStart) 로그를 까보면 도커 컨테이너가 뻗어버린 기만적인 배포 실패 화면입니다.*
...
## 4. 파인만 비유 부록
- **CodeDeploy 유령 식당**: 서빙 직원(Agent)을 EC2에 설치도 안 해놓고 본사에서 호출하니, 타임아웃으로 미쳐버려서 "옛날 요리 치우다 실패했어!"라고 존재하지도 않는 유령 보고를 던진 에러.
- **Flyway (깐깐한 금고지기)**: 위험하게 DB를 멋대로 조작하는 자동 번역기(Hibernate `ddl-auto: update`)와 달리, 개발자가 명시적으로 써준 결재 서류(`V1__init.sql`)만 딱 1번 읽어 장부에 적고 통과시키는 안전한 금고지기.
