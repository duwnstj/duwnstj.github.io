...
처음에는 보안을 위해 ECS를 **프라이빗 서브넷**에 두려 했습니다. 프라이빗 서브넷은 외부에서 직접 접근할 수 없으니 가장 확실한 물리적 방어처럼 보였습니다. 하지만 곧바로 문제에 부딪혔습니다. 프라이빗 서브넷의 ECS는 인터넷이 막혀있기 때문에, 도커 이미지를 가져오는 **ECR**과 DB 비밀번호를 꺼내오는 **SSM Parameter Store**에 접근할 수가 없었습니다. 이 문제를 해결하려면 프라이빗 서브넷과 퍼블릭 인터넷 사이를 연결해주는 **NAT Gateway(프라이빗 서브넷의 인스턴스가 인터넷으로 나가는 통로)**가 반드시 필요했습니다.

문제는 NAT Gateway의 유지비용이 **시간당 과금** 구조라 주말 실습 환경에서도 월 약 4만 원이 청구된다는 것이었습니다. FinOps 관점에서 이 비용은 감당할 수 없었고, 결국 NAT Gateway를 포기하고 ECS를 **퍼블릭 서브넷**에 배치하는 선택을 해야 했습니다. 이로 인해 새로운 딜레마가 생겼습니다. 비용은 지키되, 퍼블릭에 노출된 ECS와 RDS를 해커의 다이렉트 접근으로부터 어떻게 논리적으로 보호하느냐는 **퍼블릭 서브넷 환경에서의 망 분리 딜레마**였습니다.

## 2. Socratic Deep Dive (원인 파악)
보안의 본질을 짚어내기 위해, AI 튜터와의 대화를 통해 아키텍처를 해부했습니다.

- **나의 첫 번째 분석**: 다중 서버 환경에서 Health Check를 하려면 ALB가 필요하고, ALB가 HTTPS 암호화를 대신 해제하는 **SSL Termination(외부에서 오는 암호화 트래픽을 ALB가 대신 풀어서 EC2에게는 평문으로 넘겨주는 것)**을 담당하는 것 같다. 그리고 EC2 보안 그룹은 443 포트만 열어두면 될 것이다.
...
graph LR
    ...
    User(("🌐 외부 유저"))
    Hacker(("👿 해커"))
    ALB["ALB (안내데스크) SG-ALB (443 오픈)"]:::alb
    ECS["ECS / EC2 (실무자) Source: SG-ALB만 허용"]:::ecs

    User -- "HTTPS (443)" --> ALB
    ALB -- "HTTP / SSL Termination" --> ECS
    Hacker -. "Direct IP Blocked!" .-> ECS
```
...
의사결정 결과, NAT Gateway 없이 ALB와 백엔드 컴포넌트 간의 연결 고리를 묶는 Chaining 방식을 채택했습니다. 추가로, 무중단 배포를 위한 `create_before_destroy`(새 인증서를 먼저 완성한 뒤 기존 인증서를 삭제하는 순서 제어) 생명주기 블록을 도입하여 단 1초의 HTTPS 다운타임도 허용하지 않는 불변 인프라 로직을 적용했습니다.

## 4. Resolution & Lesson (결과 및 면접 방어)
SG Chaining 기법을 통해 **비용 발생 없이 완벽한 망 분리**를 이뤄냈습니다.

- **SRE/보안 관점**: ALB는 무거운 암호화 연산을 전담하는 **SSL Termination(암호화 해제를 ALB가 대신 처리하여 EC2 CPU를 아끼는 것)**으로 백엔드 CPU 자원을 아낍니다. 또한 SG Chaining은 단순 IP 화이트리스팅이 아닌 리소스 ID 기반의 신뢰 네트워크를 형성하므로, 동적으로 IP가 변하는 오토스케일링(ASG)이나 Fargate 환경에서도 빈틈없는 **Zero-Trust(아무도 기본적으로 신뢰하지 않고, 허가된 대상만 통과시키는 보안 원칙)** 보안을 유지합니다.
