...
AWS 환경에서 Terraform으로 인프라를 구축하며 가장 먼저 마주친 벽은 **요금 청구서**와 **프로비저닝 데드락(Deadlock, 두 작업이 서로를 기다리다 영원히 멈추는 상태)**이었습니다. 
...
- **나의 깨달음**: "아! 닭과 알의 딜레마이자 수명의 문제구나." 수명이 긴 `base`(Route53 등)와 과금이 발생하는 가변적인 `app`(ALB, ECS, RDS)으로 State를 격리하는 **State Layering(수명이 다른 인프라를 별도 폴더로 분리해 관리하는 전략)**이 답이다. 그리고 AWS 콘솔에서 마우스를 쓰지 않고 터미널 명령어로만 인프라를 파괴하는 **Zero-ClickOps**만이 완벽한 정답이다.
...
2. **대안 B: State Layering (Base / App 분리)** ⭐ **선택**
   - **장점**: 수명이 긴 것과 자주 교체되는 것을 다른 폴더로 격리. 주말마다 과금이 발생하는 `app` 디렉토리만 안전하게 `destroy`하여 비용 방어 완벽 달성. DNS는 `base`에 안전하게 남아있음.
   - **단점**: 두 폴더 간 데이터를 공유하기 위해 Remote State Data Source(한 폴더의 결과물을 다른 폴더에서 참조하는 방식)를 추가로 설정해야 하는 초기 구현 복잡도 존재.
...
- **SRE 관점**: 인프라가 꼬였을 때 콘솔에서 직접 클릭해서 수동 복구하는 게 아니라, `git checkout`으로 과거 코드 상태로 되돌린 뒤 `apply`하면 Terraform이 알아서 남은 자원을 역산해 파괴하는 **GitOps 기반 롤백 전략(코드가 곧 인프라의 진실)** 을 체화했습니다. 장애 순간에 엔지니어가 콘솔을 헤매는 시간을 없애고, 코드 한 줄로 복구 가능한 예측 가능한 인프라를 만드는 것이 목표였습니다.
