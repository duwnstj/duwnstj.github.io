---
layout: page
title: About
---

## 👨‍💻 준서 (Junseo Yeo)
> **"비용 최적화와 안정적인 아키텍처를 설계하는 FinOps / DevOps 엔지니어"**

현재 클라우드 비용 효율화를 위한 `Cover Challenge` 플랫폼을 개발 및 운영하고 있습니다.
서버리스, 컨테이너, CI/CD 자동화 파이프라인 구축에 관심이 많으며, 인프라 트러블슈팅 과정을 블로그에 기록합니다.

---

### 🛠️ Tech Stack

**Infra & Cloud**
- AWS (EC2, S3, RDS, VPC)
- Docker, Docker Compose

**CI/CD & DevOps**
- GitHub Actions

**Backend**
- Spring Boot 3.x
- Java 17

---

### 🚀 Projects

#### **Cover Challenge Platform**
- **역할:** DevOps & Backend Engineer
- **내용:** Spring Boot + Docker + AWS 기반 플랫폼 아키텍처 설계 및 구축
- **성과:** GitHub Actions를 활용한 무중단 배포 파이프라인 구축 및 FinOps 기반 아키텍처 비용 최적화

---

### 📝 Certifications
- *(취득 예정 자격증이나 이수한 교육을 여기에 기록하세요)*

---

### 📫 Contact
- **Email:** duwnstj12@naver.com
- **GitHub:** [https://github.com/duwnstj](https://github.com/duwnstj)
